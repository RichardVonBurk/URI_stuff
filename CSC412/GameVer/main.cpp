...
Traveler* controlledTraveler; //set controlled traveler here for easy access
...
void progressively_delete_traveler(Traveler* travelerPtr)
{
	...
	while(!travelerPtr->segmentList.empty())
	{
		...
		grid[ travelerPtr->segmentList[travelerPtr->segmentList.size()-1].row ]
			[ travelerPtr->segmentList[travelerPtr->segmentList.size()-1].col ] = FREE_SQUARE;
		//remove segment
...
void* updateTraveler(void* voidPtr)
{
	...
	{
		if(travelerPtr->isControlled!=true)
		{	
		...
		//is being controlled by player
		else
		{
			while(travelerPtr->isControlled == true)
			{
				usleep(travelerSleepTime/2);
				//loop to wait for keyboard input with some delay
				//wait a fraction of a sec so user doesn't become Sonic the Hedgehog
			}
		}
		usleep(travelerSleepTime);
	}
...
void handleKeyboardEvent(unsigned char c, int x, int y)
{
	...
	switch (c)
	{
		...
		//North
		case 'w':
			if (controlledTraveler->segmentList[0].row != numRows-1)
			{
				if(grid[controlledTraveler->segmentList[0].row-1][controlledTraveler->segmentList[0].col] == FREE_SQUARE)
				{
					grid[controlledTraveler->segmentList[0].row][controlledTraveler->segmentList[0].col] = FREE_SQUARE;
					grid[controlledTraveler->segmentList[0].row-1][controlledTraveler->segmentList[0].col] = TRAVELER;
					controlledTraveler->segmentList[0].row-=1;
				}
				else if(grid[controlledTraveler->segmentList[0].row-1][controlledTraveler->segmentList[0].col] == EXIT)
				{
					progressively_delete_traveler(controlledTraveler);
					numTravelersDone++;
					pthread_exit(NULL);
				}
			}
		...
		//West
		case 'a':
			if (controlledTraveler->segmentList[0].col != numCols-1)
			{
				if(grid[controlledTraveler->segmentList[0].row][controlledTraveler->segmentList[0].col-1] == FREE_SQUARE)
				{
					grid[controlledTraveler->segmentList[0].row][controlledTraveler->segmentList[0].col] = FREE_SQUARE;
					grid[controlledTraveler->segmentList[0].row][controlledTraveler->segmentList[0].col-1] = TRAVELER;
					controlledTraveler->segmentList[0].col-=1;
				}
				else if(grid[controlledTraveler->segmentList[0].row][controlledTraveler->segmentList[0].col-1] == EXIT)
				{
					progressively_delete_traveler(controlledTraveler);
					numTravelersDone++;
					pthread_exit(NULL);
		...
		case 's':
			//check if near edge
			if (controlledTraveler->segmentList[0].row != 0)
			{
				//if free square
				if(grid[controlledTraveler->segmentList[0].row+1][controlledTraveler->segmentList[0].col] == FREE_SQUARE)
				{
		...
		//East
		case 'd':
			if (controlledTraveler->segmentList[0].col != 0)
			{
				if(grid[controlledTraveler->segmentList[0].row-1][controlledTraveler->segmentList[0].col] == FREE_SQUARE)
				{
					grid[controlledTraveler->segmentList[0].row][controlledTraveler->segmentList[0].col] = FREE_SQUARE;
			...
			break;
		//select traveler to control by releasing current controlledTravel then moving to another
		case '0':
			controlledTraveler->isControlled = false;
			controlledTraveler = &travelerList[0];
			controlledTraveler->isControlled = true;
		case '1':
			controlledTraveler->isControlled = false;
			controlledTraveler = &travelerList[1];
			controlledTraveler->isControlled = true;
		case '2':
			controlledTraveler->isControlled = false;
			controlledTraveler = &travelerList[2];
			controlledTraveler->isControlled = true;
		case '3':
			controlledTraveler->isControlled = false;
			controlledTraveler = &travelerList[3];
			controlledTraveler->isControlled = true;
		case '4':
			controlledTraveler->isControlled = false;
			controlledTraveler = &travelerList[4];
			controlledTraveler->isControlled = true;
		case '5':
			controlledTraveler->isControlled = false;
			controlledTraveler = &travelerList[5];
			controlledTraveler->isControlled = true;
		case '6':
			controlledTraveler->isControlled = false;
			controlledTraveler = &travelerList[6];
			controlledTraveler->isControlled = true;
		case '7':
			controlledTraveler->isControlled = false;
			controlledTraveler = &travelerList[7];
			controlledTraveler->isControlled = true;
		case '8':
			controlledTraveler->isControlled = false;
			controlledTraveler = &travelerList[8];
			controlledTraveler->isControlled = true;
		case '9':
			controlledTraveler->isControlled = false;
			controlledTraveler = &travelerList[9];
			controlledTraveler->isControlled = true;
...
{
	// can't seem to get argv & argc to work on this assignment
	// first time this whole semester really
	...
	numTravelers = 10;	//game version gets 10 travelers
	numLiveThreads = 0;
	numTravelersDone = 0;
...
void initializeApplication(void)
{
	...
	//initialize game version so traveler[0] is controlled
	travelerList[0].isControlled = true;
	controlledTraveler = &travelerList[0];
