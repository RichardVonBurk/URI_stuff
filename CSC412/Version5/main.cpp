...
pthread_mutex_t colsLocks[];		//Version5 locks
pthread_mutex_t rowsLocks[];
	...
	while (keepGoing==true)
	{
		...
		switch (travelerPtr->segmentList[0].dir)
		{
			case NORTH:
				...
				{
					pthread_mutex_lock(&rowsLocks[travelerPtr->segmentList[0].row+1]);
					pthread_mutex_lock(&colsLocks[travelerPtr->segmentList[0].col]);
					...
					pthread_mutex_unlock(&rowsLocks[travelerPtr->segmentList[0].row+1]);
					pthread_mutex_unlock(&colsLocks[travelerPtr->segmentList[0].col]);
					updateSegments(&travelerPtr);
				}
			...
			case WEST:
				...
				{
					pthread_mutex_lock(&rowsLocks[travelerPtr->segmentList[0].row]);
					pthread_mutex_lock(&colsLocks[travelerPtr->segmentList[0].col+1]);
					grid[travelerPtr->segmentList[0].row][travelerPtr->segmentList[0].col] = FREE_SQUARE;
					grid[travelerPtr->segmentList[0].row][travelerPtr->segmentList[0].col+1] = TRAVELER;
					travelerPtr->segmentList[0].col+=1;
					pthread_mutex_unlock(&rowsLocks[travelerPtr->segmentList[0].row]);
					pthread_mutex_unlock(&colsLocks[travelerPtr->segmentList[0].col+1]);
					updateSegments(&travelerPtr);
				}				
				//	change to new direction
				else
